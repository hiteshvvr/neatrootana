...
void massselect::CreateHistograms() {
    ...
    TH1D *masshist = new TH1D("masshist", "Mass Histogram", 1000, 0, 50);
    ...
    // TH2D *hmol = new TH2D("hmol", "Hydrogen Molecule", 50, 0.5,0.65, 50,0.5,0.6);
    TH2D *hmol= new TH2D("hmol", "HMol", 200,0,1,200,0,1);
    hmol->SetXTitle("X Position");
    hmol->SetYTitle("Y Position");
    push_back(hmol);

    TH2D *ohmol = new TH2D("ohmol", "OH Molecule", 50, 0.5,0.6, 50,0.4,0.6);
    ohmol->SetXTitle("X Position");
    ohmol->SetYTitle("Y Position");
    push_back(ohmol);

    // TH2D *water= new TH2D("water", "Water", 50, 0.5,0.6, 50,0.5,0.6);
    TH2D *water= new TH2D("water", "water", 200,0,1,200,0,1);
    water->SetXTitle("X Position");
    water->SetYTitle("Y Position");
    push_back(water);

    // TH2D *neon = new TH2D("neon", "Neon", 50, 0,1, 50,0,1);
    TH2D *neon= new TH2D("neon", "Neon", 200,0,1,200,0,1);
    neon->SetXTitle("X Position");
    neon->SetYTitle("Y Position");
    push_back(neon);

    // TH2D *neondim = new TH2D("neondim", "Neon Dimer", 50, 0,1, 50,0,1);
    TH2D *neondim= new TH2D("neondim", "NeonDim", 200,0,1,200,0,1);
    neondim->SetXTitle("X Position");
    neondim->SetYTitle("Y Position");
    push_back(neondim);


    TH1D *timehist = new TH1D("timehist", "Mass Histogram", 6000, 0, 6000);
    masshist->SetXTitle("Mass in amu");
    masshist->SetYTitle("Counts");
    push_back(timehist);
...
void massselect::UpdateHistograms(TDataContainer &dataContainer) {
    ...
    for (int i = 0; i < numhit-1; i++) // loop over measurements
    {
        int chan0 = measurements[i].GetChannel();
        int chan1= measurements[i+1].GetChannel();
        if(chan0 == startchannel && chan1==stopchannel){
            startdata = measurements[i].GetMeasurement() * RES_1290N;
            stopdata  = measurements[i+1].GetMeasurement() * RES_1290N;
            goodevt = 1;
        }
    }
    float tdiff = stopdata - startdata;
    ...
    if (v1720 && !v1720->IsZLECompressed()) {
        ...
        int windowmin = 500 , windowmax = 1700;
        for (k = 0; k < numsam ; k++) {
        ...
        if(windowmin < smaxpos && smaxpos < windowmax)
        {
            ...
            if (doubleeventflag == 0)
            {
                ...
                double mass = 0;
                double slope = 0.001702;
                double intercept = -0.1375;


                mass = tdiff*slope + intercept;
                mass = mass*mass;
