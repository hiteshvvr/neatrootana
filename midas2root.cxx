...
class Analyzer : public TRootanaEventLoop
{
    public:
        ...
        struct simpleneateventdata{
            ...
            UInt_t tdc_chan0Data;
            UInt_t tdc_chan1Data;
            float tdc_tdiff;
            int digi_midasid;
            int digi_eventid;
        ...
        struct neateventdata{
            ...
            unsigned int tdc_timestampdata[5];
            int tdc_channodata[5];
            uint32_t tdc_chan0Data;
            uint32_t tdc_chan1Data;
            float tdc_tdiff;
            int digi_midasid;
            int digi_eventid;
            uint32_t digi_timetag;
        ...
        TH1D *mhits = new TH1D("multiHit", "multiHit", 100000, -5e5, 5e5);
        TH1D *triples2ion = new TH1D("triples2ion", "Triple2ions", 5000, -7000, 7000);
        TH1D *triples2electrs = new TH1D("triples2electrs", "Triple2Electrons", 5000, -7000, 7000);
        ...
        void BeginRun(int transition, int run, int time)
        {
            ...
            neat->Branch("tdc_gevnt", &event.gevnt, "tdc_gevnt/I");
        ...
        void EndRun(int transition, int run, int time) {
            ...
            triples2ion->Write("tripleswithtwoions");
            triples2electrs->Write("tripleswithtwoelectrons");
            tripdis->Write("triplesdistribution");
#endif
        ...
        bool ProcessMidasEvent(TDataContainer &dataContainer)
        {
            ...
            // if (midasid % 10000 == 0) printf("%d");
            // int i,k;
            // if (midasid % 10000 == 0) std::cout<< "midasid:\t" << midasid << std::endl;
            ...
            if (v1290data) {
                ...
                for (i = 0; i < numhit ; i++)
                {
                    ...
                    unsigned int chandata = measurements[i].GetMeasurement() * RES_1290N;
                    ...
                    event.tdiff = stoptime - starttime;
                    event.gevnt = 0;
                }
                hitcounts->Fill(numhit * 10 + stopchancount);
                nuofhits->Fill(numhit);
                for (i= 0; i<numhit-1; i++){
                    if(event.channodata[i] == 0 && event.channodata[i+1] == 1){
                        event.chan0Data = event.timestampdata[i];
                        starttime = event.timestampdata[i];
                        event.chan1Data = event.timestampdata[i+1];
                        stoptime = event.timestampdata[i+1];
                        event.tdiff = stoptime - starttime;
                        if(event.timestampdata[i+1]>event.timestampdata[i]) event.gevnt = 1;
                            // event.timestampdata[i+1] = event.timestampdata[i+1] + 2097152*RES_1290N; // this number is 2^21, this is needed due to circular memeory in tdc.
                        break;
                    }
                }
                float temptdiff = 0;
                ...
                if (numhit > 3)
                {
                    for (i = 0; i < numhit; i++)
                    {
                        ...
                        if (chan != stopchannel)
                        {
                            float mtdiff = stoptime - measurements[i].GetMeasurement()*RES_1290N;
                            mhits->Fill(mtdiff);
